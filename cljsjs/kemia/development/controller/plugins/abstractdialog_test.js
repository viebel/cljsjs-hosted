goog.provide('kemia.controller.plugins.AbstractDialogTest');
...
goog.require('kemia.controller.plugins.AbstractDialog');
goog.require('kemia.controller.plugins.AbstractDialog.Builder');
goog.require('kemia.controller.plugins.AbstractDialog.EventType');
...
function setUp() {
	...
	mockCustomButtonHandler = mockCtrl
			.createLooseMock(goog.events.EventHandler);
}
...
function expectAfterHide() {
	mockAfterHideHandler
			.handleEvent(new goog.testing.mockmatchers.ArgumentMatcher(
					function(arg) {
						return arg.type == kemia.controller.plugins.AbstractDialog.EventType.AFTER_HIDE;
					}));
}
...
function expectOk() {
	mockOkHandler
			.handleEvent(new goog.testing.mockmatchers.ArgumentMatcher(
					function(arg) {
						return arg.type == kemia.controller.plugins.AbstractDialog.EventType.OK;
					}));
}
...
/**
 * Sets up the mock event handler to expect an OK event and to return false when
 * handling it.
 */
...
function expectCancel() {
	mockCancelHandler
			.handleEvent(new goog.testing.mockmatchers.ArgumentMatcher(
					function(arg) {
						return arg.type == kemia.controller.plugins.AbstractDialog.EventType.CANCEL;
					}));
}
...
function createTestDialog() {
	var dialog = new kemia.controller.plugins.AbstractDialog(
			new goog.dom.DomHelper());
	builder = new kemia.controller.plugins.AbstractDialog.Builder(dialog);
	...
	dialog.addEventListener(
			kemia.controller.plugins.AbstractDialog.EventType.AFTER_HIDE,
			mockAfterHideHandler);
	dialog
			.addEventListener(
					kemia.controller.plugins.AbstractDialog.EventType.OK,
					mockOkHandler);
	dialog.addEventListener(
			kemia.controller.plugins.AbstractDialog.EventType.CANCEL,
			mockCancelHandler);
...
function testBasicLayout() {
	...
	builder.setTitle('The Title').setContent(content).addOkButton(
			'The OK Button').addCancelButton().addButton('The Apply Button',
			goog.nullFunction).addClassName('myClassName');
	dialog.show();

	var dialogElem = dialog.dialogInternal_.getElement();
	var html = dialogElem.innerHTML;
	// TODO: This is really insufficient. If the title and content
	...
	assertTrue('Dialog should have default Closure class', goog.dom.classes
			.has(dialogElem, 'modal-dialog'));
	assertTrue('Dialog should have our custom class', goog.dom.classes.has(
			dialogElem, 'myClassName'));
...
/**
 * Tests that hitting the escape key dispatches the CANCEL event and closes the
 * dialog (dispatching the AFTER_HIDE event too).
 */
function testEscape() {
	dialog = createTestDialog();
...
function testCustomButton() {
	...
	goog.testing.events.fireClickSequence(dialog
			.getButtonElement(CUSTOM_BUTTON_ID));
...
/**
 * Tests that processOkAndClose() dispatches the OK event and closes the dialog
 * (dispatching the AFTER_HIDE event too).
 */
